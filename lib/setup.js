...
async function createTables(db, results) {
  ...
  await db`CREATE INDEX IF NOT EXISTS idx_messages_session ON chat_messages(session_id)`;
...
async function seedDemoData(db, results) {
  ...
  const customerRows = await db`
    ...
    VALUES ('ElderCare Pilot', 'eldercare-pilot', ${orgId}, 'trial')
  ...
  const astridPrompt = securityPrompt + getAstridPrompt();
  
  await db`
    INSERT INTO customer_ai_config (customer_id, ai_name, ai_role, greeting, language, system_prompt)
    VALUES (${customerId}, 'Astrid', 'Samtalepartner for eldre', 
  ...
  await db`
    INSERT INTO customer_companions (
      ...
      ${customerId}, 'astrid', 'Astrid', 'Vennlig samtalepartner', '👵',
      'Hei! Jeg heter Astrid 👵 Så hyggelig å snakke med deg. Hvordan har du det i dag?',
      ${astridPrompt}, 'Varm, omsorgsfull, tålmodig', 'no', true, true
    )
  ...
  const ivarPrompt = securityPrompt + getIvarPrompt();
  await db`
    INSERT INTO customer_companions (
      customer_id, companion_key, ai_name, ai_role, avatar, greeting,
      system_prompt, personality, language, is_default, is_active
    ) VALUES (
      ${customerId}, 'ivar', 'Ivar', 'Vennlig samtalepartner', '👴',
      'Hei der! Ivar her 👴 Hyggelig å treffe deg. Hva tenker du på i dag?',
      ${ivarPrompt}, 'Jovial, jordnær, humoristisk', 'no', false, true
    )
    ON CONFLICT (customer_id, companion_key) DO UPDATE SET
      system_prompt = EXCLUDED.system_prompt,
      greeting = EXCLUDED.greeting
  ...
  await db`
    INSERT INTO customer_analysis_config (
      ...
      ${customerId}, true, 'eric@eryai.tech', 'astrid@eryai.tech', 'Astrid',
